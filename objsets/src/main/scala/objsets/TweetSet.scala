// Read https://chatgpt.com/share/678d3810-2820-800a-b43f-d7538e42e29c
// to understand more about difference between
// left.union(right.union(that)).incl(elem) and
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    if p(elem) then filtered.incl(elem) else filtered
  }
  ...
  def mostRetweeted: Tweet = {
    ...
    if elem.retweets > leftMost.retweets then
      if elem.retweets > rightMost.retweets then elem else rightMost
    else if leftMost.retweets > rightMost.retweets then leftMost else rightMost
  }
