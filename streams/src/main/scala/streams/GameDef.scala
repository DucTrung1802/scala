...
trait GameDef:
  ...
  def startBlock: Block =
    val b1 = Pos(startPos.row, startPos.col)
    val b2 = Pos(startPos.row, startPos.col)
    Block(b1, b2)
  ...
  case class Block(b1: Pos, b2: Pos):
    ...
    def neighbors: List[(Block, Move)] =
      List(
        (Block(b1, b2).left, Move.Left),
        (Block(b1, b2).right, Move.Right),
        (Block(b1, b2).up, Move.Up),
        (Block(b1, b2).down, Move.Down)
      )
