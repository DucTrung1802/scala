...
object Anagrams extends AnagramsInterface:
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = s match {
    case List() | Nil => List()
    case head :: other =>
      (wordOccurrences(head) ::: sentenceOccurrences(other))
        .groupBy(_._1)
        .map((char, occurrence_list) => (char, occurrence_list.map(_._2).sum))
        .toList
        .sortBy(x => x._1)
  }
    ...
    occurrences match {
      ...
      case head :: other =>
        val currentCombinations =
          (1 to head._2).map(n => List((head._1, n))).toList
        val restCombinations = combinations(other)
        restCombinations ::: (for {
          subset <- restCombinations
          elem <- currentCombinations
        } yield elem ::: subset)
    }
    ...
    def anagrams(occurrences: Occurrences): List[Sentence] =
      if occurrences.isEmpty then List(List())
        ...
        for
          ...
          rest <- anagrams(subtract(occurrences, subset))
        yield word :: rest
