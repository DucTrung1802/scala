...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def scoredPostings(
    ...
    grouped.flatMap { case (_, qaPairs) =>
      qaPairs.headOption.map { case (question, _) =>
  ...
  def vectorPostings(
    ...
    scored
      .flatMap { case (question, highScore) =>
        firstLangInTag(question.tags, langs).map(langIndex =>
  ...
  @tailrec final def kmeans(
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] =
      grouped.mapValues(averageVectors).collectAsMap()
  ...
  def clusterResults(
    ...
    val median = closestGrouped.mapValues { vs =>
      val langCounts = vs.groupBy(_._1).view.mapValues(_.size)
      val (mostCommonLang, mostCommonLangCount) = langCounts.maxBy(_._2)
      val langLabel: String = langs(
        mostCommonLang / langSpread
      ) // most common language in the cluster

      val clusterSize: Int = vs.size
      val langPercent: Double =
        mostCommonLangCount * 100.0 / clusterSize // percent of the questions in the most common language

      val sortedScores = vs.map(_._2).toArray.sorted
      val medianScore: Int =
        if (sortedScores.length % 2 == 0)
          (sortedScores(sortedScores.length / 2 - 1) + sortedScores(
