...
trait Solver extends GameDef:
  ...
  def from(
    ...
    case LazyList() =>
      LazyList() // This stops recursion when there are no more paths to explore.
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(
    initial = LazyList((startBlock, List[Move]())),
    explored = Set[Block]()
  )
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =
    from(pathsFromStart, Set()).filter { case (block, _) => done(block) }
